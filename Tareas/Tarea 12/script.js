...
function eliminar(id) {
  ...
  document.querySelector("#contenido-modal").innerHTML = `
    ...
    <div class="modal-buttons">
      ...
      <button onclick="cancelarEliminacion()" class="btn-cancelar">Cancelar</button>
    </div>
  `;
  modal.style.display = "block";
}
...
function renderizarTablaRead(objeto) {
  ...
  if(nivel == 1) {
    html += '<a href="javaScript:formInsertar()"> Insertar</a>';
  }
  ...
  for (var i = 0; i < lista.length; i++) {
    ...
    if(nivel == 1) {
      html += `<td>
        <a href="javascript:editar(${lista[i].id})">Editar</a>
        <a href="javascript:eliminar(${lista[i].id})">Eliminar</a>
      </td>`;
    }
  ...
  if(nropaginas > 1) {
    ...
    if(pagina > 1) {
      html += `<li style="margin:5px"><a href="javascript:cargarPagina(1,'${buscar}','${orden}','${asente}')">&lt;&lt;</a></li>
               <li style="margin:5px"><a href="javascript:cargarPagina(${pagina-1},'${buscar}','${orden}','${asente}')">&lt;</a></li>`;
    }

    for (let i = Math.max(1, pagina-2); i <= Math.min(nropaginas, pagina+2); i++) {
      html += `<li style="margin:5px"><a href="javascript:cargarPagina(${i},'${buscar}','${orden}','${asente}')" 
               ${i === pagina ? 'style="font-weight:bold"' : ''}>${i}</a></li>`;
    }

    if(pagina < nropaginas) {
      html += `<li style="margin:5px"><a href="javascript:cargarPagina(${pagina+1},'${buscar}','${orden}','${asente}')">&gt;</a></li>
               <li style="margin:5px"><a href="javascript:cargarPagina(${nropaginas},'${buscar}','${orden}','${asente}')">&gt;&gt;</a></li>`;
    }
...
function renderizarTablaProfesiones(objeto) {
    ...
    for (var i = 0; i < lista.length; i++) {
        ...
        if(nivel == 1) {
            html += `<td>
                <a href="javascript:editarProfesion(${lista[i].id})">Editar</a>
                <a href="javascript:eliminarProfesion(${lista[i].id})">Eliminar</a>
            </td>`;
        }
